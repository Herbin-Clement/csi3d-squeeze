...
def is_valid_triangle(v1, v2, w, graph):
    # Extraire les positions des sommets v1, v2 et w
    pos_v1 = np.array(graph.nodes[v1]['pos'])
    pos_v2 = np.array(graph.nodes[v2]['pos'])
    pos_w = np.array(graph.nodes[w]['pos'])
    
    # Calculer les distances et vérifier la condition du triangle
    return np.linalg.norm(pos_v1 - pos_v2) + np.linalg.norm(pos_v2 - pos_w) > np.linalg.norm(pos_v1 - pos_w)
...
def compress_model(graph):
    ...
    for edge in edges:
        ...
        for w in neighbors:
            if w != v1 and w != v2 and not is_valid_triangle(v1, v2, w, graph):
                valid = False
                break
